...
class Bouton:
    def __init__(self, surface,x,y,width,height, etat='', id='', 
    function=0, colour=(255,255,255), hover_colour=(255,255,255),
    border =True, border_width=2, border_colour=(0,0,0), 
    text='',
    font_name='arial', text_size=20,
    text_colour=(0,0,0), bold_text=True):
    ...
    def update(self, pos, game_etat=''):
        
        if self.mouse_hovering(pos):
            self.hovered = True
        else:
            self.hovered = False
        if self.etat == '' or game_etat == '':
            self.showing = True
        else:
            if self.etat == game_etat:
                self.showing = True
            else:
                self.showing = False
